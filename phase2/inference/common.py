...
def _get_variable(name,
    ...
    "resnet collection"
    collections = [tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, RESNET_VARIABLES]
...
def batchNormalization(x, is_training= True, decay= 0.9, epsilon= 0.001):
    ...
    if is_training:
      mean, variance = tf.nn.moments(x=x, axes=axis)
...
def fullyConnected(x, num_units_out, wd= 0.0, weight_initializer= None, bias_initializer= None):
    ...
    if weight_initializer is None:
      weight_initializer = tf.compat.v1.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.float32)
    if bias_initializer is None:
      bias_initializer = tf.compat.v1.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.float32) 

    weights = _get_variable('weights',
                            [num_units_in, num_units_out], weight_initializer, tf.keras.regularizers.l2(0.5 * (wd)))

    biases = _get_variable('biases',
                           [num_units_out], bias_initializer)
                           
    return tf.compat.v1.nn.xw_plus_b(x, weights, biases)
...
def spatialConvolution(x, ksize, stride, filters_out, wd= 0.0, weight_initializer= None, bias_initializer= None):
    ...
    weights = _get_variable('weights',
                            shape, weight_initializer, tf.keras.regularizers.l2(0.5 * (wd)))

    conv = tf.nn.conv2d(input=x, filters=weights, strides=[1, stride, stride, 1], padding= 'SAME')
...
def resnetStack(x, num_blocks, stack_stride, block_filters_internal, bottleneck, wd= 0.0, is_training= True):
    for n in range(num_blocks):
        ...
        block_stride = s
        with tf.compat.v1.variable_scope('block%d' % (n + 1)):
